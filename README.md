## OPC DA/AE Server Base

### Introduction
The OPC DA/AE Server Base offers a fast and easy access to the OPC Data Access (DA) and OPC Alarms&Events (AE) technology. Develop OPC DA 2.05a, 3.00 00 and OPC AE 1.00, 1.10 compliant Servers with any compiler capable of either

- generating a Windows DLL (OPC DA/AE Server Base DLL). This results in a generic server executable plus a Windows DLL.
- generating a .NET 4.8 assembly (OPC DA/AE Server Base .NET). This results in a generic server executable plus a .NET 4.8 assembly.
