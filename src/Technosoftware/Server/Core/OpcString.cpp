...
COpcString& COpcString::operator+=(const COpcString& cStr)
{
    TStrBuf* pBuf = Alloc(GetLength() + cStr.GetLength());
...
UINT COpcString::GetLength() const
{
    ...
    #ifdef _UNICODE
    return (UINT)wcslen(m_pBuf->wszStr);
    #else
...
int COpcString::Find(LPCTSTR tsTarget) const
{
    ...
    int iLength = (int)_tcslen(tsTarget);
    int iLast   = (int)_tcslen(tsStr) - iLength + 1;
...
int COpcString::ReverseFind(LPCTSTR tsTarget) const
{
    ...
    int iLength = (int)_tcslen(tsTarget);
    int iLast   = (int)_tcslen(tsStr) - iLength;
...
COpcString& COpcString::Trim()
{
    ...
    for (UINT ii = uEnd-1; ii >= uStart; ii--)
    {
        if (!_istspace((*this)[ii]))
        {
            uEnd = ii+1;
