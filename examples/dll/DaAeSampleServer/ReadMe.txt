...
- OpcDllDaAeServer.exe
    This is the generic OPC DA 2.05a/3.00 and AE 1.00/1.10 server

Post Build Steps
After a successful compilation the following steps are executed in the post 
build event:
1. copy the files OpcDllDaAeServer.exe from the project directory 
   into the bin directory
2. Register the server by executing OpcDllDaAeServer.exe -regserver
					
The server is now registered and can be accessed by OPC DA 1.0a/2.05a/3.00 
and AE 1.00/1.10 clients.

Debugging
To debug the plug-in assembly you need to:
1.  Open project properties and select  
		Configuration Properties  -  Debugging
		Select 'Start external program' and browse to OpcDllDaAeServer.exe 
		in bin
2.  Set Breakpoints
